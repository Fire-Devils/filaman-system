...
class Filament(Base, TimestampMixin):
    ...
    material_type: Mapped[str] = mapped_column(
        "type", String(50), nullable=False, index=True
    )
    material_subgroup: Mapped[str | None] = mapped_column(
        String(100), nullable=True, index=True
    )
    diameter_mm: Mapped[float] = mapped_column(Float, nullable=False)

    manufacturer_color_name: Mapped[str | None] = mapped_column(
    ...
    printer_profiles: Mapped[list["FilamentPrinterProfile"]] = relationship(
        back_populates="filament", cascade="all, delete-orphan"
    )
    ratings: Mapped[list["FilamentRating"]] = relationship(
        back_populates="filament", cascade="all, delete-orphan"
    )
    printer_params: Mapped[list["FilamentPrinterParam"]] = relationship(
        back_populates="filament", cascade="all, delete-orphan"
    )
...
class FilamentColor(Base):
    ...
    color_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("colors.id"), nullable=False, index=True
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "filament_id", "position", name="uq_filament_colors_filament_position"
        ),
    )
...
class FilamentRating(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    filament_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("filaments.id", ondelete="CASCADE"), nullable=False
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "filament_id", "user_id", name="uq_filament_ratings_filament_user"
        ),
    )
...
class FilamentPrinterProfile(Base, TimestampMixin):
    ...
    max_volumetric_flow_mm3_s: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )
    ...
    retraction_extra_prime_mm3: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )
