...
def _validate_presets(
    presets: Sequence[LabelPresetInput], *, reject_duplicates: bool = True
) -> None:
...
async def upsert_label_preset(
    ...
    async with _preset_transaction(db):
        ...
        if body.create_only and preset is not None:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail={
                    "code": "preset_conflict",
...
async def migrate_label_presets(
    ...
    async with _preset_transaction(db):
        ...
        existing = {
            (preset_type, name_key)
            for preset_type, name_key in existing_result.tuples()
        }
        counts: dict[str, int] = {}
